...
Use `pal-dev` to run PAL from source against any config directory:

```bash
cd ~/devel/pulp/pal-config && pal-dev     # run from source
cd ~/devel/alcove && pal-dev              # works with any project
PAL_BIN=pal-dev brian-po                  # named agent from source
```

`pal-dev` saves the current directory, cd's to the PAL source tree, and runs `bun run dev` with the config dir as a positional argument. Wrapper scripts generated by `pal init` use `${PAL_BIN:-pal}`, so setting `PAL_BIN=pal-dev` switches them to dev mode.

### Releasing

Pushes to `main` build binaries (CI validation) but do NOT publish. To release:
...
`make release` reads the version from `package.json`, creates a git tag, and pushes it. CI builds and publishes to packages.redhat.com on tag push only.
