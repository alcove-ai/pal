...
function tierBadge(tier: 1 | 2): string {
  return tier === 1 ? "T1" : "T2"
}
...
function scoreColor(score: number, theme: any): string {
  if (score >= 60) return theme.error ?? theme.primary
  if (score >= 35) return theme.warning ?? theme.primary
  ...
  try {
    ...
    return Database.use((db) => {
      const rows = db
        ...
        .from(DismissedEventTable)
        .where(and(eq(DismissedEventTable.action, "snooze"), gt(DismissedEventTable.snooze_until, now)))
        .all()
  ...
  try {
    return Database.use((db) => {
      const result = db
        ...
        .from(DismissedEventTable)
        .where(and(eq(DismissedEventTable.rule_source, ruleSource), eq(DismissedEventTable.action, "dismiss")))
        .get()
...
function computeFilteredQueue(config: NeedsMeConfig): NeedsMeItem[] {
  ...
  return items.filter((item) => {
    ...
    if (suppressed.has(item.ruleSource) && !item.isExemptFromSuppression) return false
    return true
  })
}
...
export function NeedsMe() {
  ...
  function refresh() {
    const items = computeFilteredQueue(config)
    setQueue(items)
    ...
    if (items.length > OVERFLOW_THRESHOLD) {
      ...
      if (since === null) {
        setOverflowSince(now)
        setOverflowAlert(false)
      } else if (now - since >= OVERFLOW_SUSTAIN_MS) {
        setOverflowAlert(true)
      }
    } else {
      setOverflowSince(null)
      setOverflowAlert(false)
    }
  }
  ...
  function handleDismiss(item: NeedsMeItem) {
    ...
    if (count >= AUTO_SUPPRESS_THRESHOLD) {
      upsertSuppression(item.ruleSource, count)
    }
    refresh()
  }
  ...
  function handleSnooze(item: NeedsMeItem, duration: SnoozeDuration) {
    ...
    insertDismissal(item.workItemKey, item.ruleSource, "snooze", snoozeUntil)
    refresh()
  }
  ...
  const visibleItems = () => {
    ...
    if (overflowAlert() && all.length > OVERFLOW_THRESHOLD) {
      const pinned = all.slice(0, 10)
      const rest = all.slice(10)
      return { pinned, collapsed: rest, collapsedCount: rest.length }
    }
      ...
      <Show when={overflowAlert()}>
        <box height={1} flexShrink={0} paddingLeft={1}>
          <text fg={theme.error} attributes={TextAttributes.BOLD}>
            {"Your queue has had >"}{OVERFLOW_THRESHOLD}{" items for 2h+ -- consider bulk-triaging or delegating."}
          </text>
        </box>
      </Show>
      <box height={1} flexShrink={0} paddingLeft={1} flexDirection="row">
